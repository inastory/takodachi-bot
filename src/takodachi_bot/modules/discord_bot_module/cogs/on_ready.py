...
class OnReady(Cog):
    ...
    async def on_ready(self):
        print("--------")
        print(
            "Good morning, after noon, evening! It is tako time. Wah! How are you today?"
        )
        print("--------")
        print(
            "Logged in as "
        ...
        print(
            "https://discordapp.com/oauth2/authorize?self.bot_id={}&scope=self.bot&permissions=8".format(
                self.bot.user.id
            )
        )
        print("--------")
        ...
        return await self.bot.change_presence(
            activity=Activity(type=ActivityType.watching, name="Ina's back")
        )
